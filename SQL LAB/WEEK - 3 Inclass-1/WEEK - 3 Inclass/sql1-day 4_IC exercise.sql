...
select c.customer_Id,c.customer_name,avg(b.Balance_amount)
from bank_customer c
join bank_account_details b
...
Select distinct customer_id , ba.account_number,ba.Account_type,
	Case when (ba.Account_type LIKE "%Credit%" and (ifnull(balance_amount,0)=0)) 
...
select b.account_number,b.balance_amount,t.transaction_amount,t.Transaction_Date,month(t.Transaction_Date)
from bank_account_details b
join bank_account_transaction t
on t.Account_Number=b.Account_Number
where month(t.Transaction_Date) between 3 and 4 and year(t.Transaction_Date)=2020;
...
select b.customer_id,a.account_number,balance_amount,t.transaction_amount,t.Transaction_date
...
join bank_account_transaction t
on t.Account_Number=a.Account_Number
where date_format(t.Transaction_Date,'%Y-%m') != '2020-03';
...
select b.customer_id,b.account_number,balance_amount,transaction_amount,transaction_date
from bank_account_details b
inner join bank_account_transaction t
...
from bank_customer_messages bm
cross join bank_account_details ba
...
select ba.Customer_id, ba.Account_Number,(Balance_amount + IFNULL(transaction_amount, 0)) deducted_balance_amount
from Bank_Account_Details ba
left join bank_account_transaction bat 
on ba.account_number = bat.account_number and Relationship_type = "P";
...
SELECT ba.Account_Number as primary_account_number , ba.Account_type as primary_account_type,
 br.Account_Number as secondary_account_number, br.Account_type as secondary_account_type,  bt.Transaction_amount   as primary_acct_tran_amount
from bank_Account_Details ba
JOIN bank_account_relationship_details br
on ba.Account_Number = br.Linking_Account_Number
...
SELECT  ba.Account_Number  as primary_account_number ,ba.Account_type as primary_account_type,
br.Account_Number as secondary_account_number,br.Account_type as secondary_account_type
from bank_Account_Details ba
JOIN bank_account_relationship_details br
on ba.Account_Number = br.Linking_Account_Number
