...
select round(avg(estimated_sale_price),2) as average from bank_inventory_pricing;
...
select Product,count(distinct Product) from Bank_inventory_pricing;
...
select bank_month,sum(Purchase_cost) from Bank_inventory_pricing
where bank_month in (1,2)
group by bank_month;
...
select Product,avg(ifnull(purchase_cost,0)) from Bank_inventory_pricing
group by Product
...
select Product,avg(ifnull(estimated_sale_price,purchase_cost)) from Bank_inventory_pricing
group by Product;

# Question 20:
# 20) Print products and their avg price on condition that products appeared in at least three different months.

select Product,avg(price),count(distinct bank_month) from Bank_inventory_pricing
group by Product
having count(distinct bank_month) >= 2;
