...
select Product,avg(Estimated_profit) from Bank_branch_PL
group by Product;
...
select Month,sum(Quantity) from Bank_Inventory_pricing
group by Month;
...
select Product,max(Estimated_sale_price-Purchase_cost)
...
select Product,min(revenue) from Bank_branch_PL
...
select Product,month,sum(Quantity) from Bank_Inventory_pricing
group by Product
...
# 7) IF Real profit is calculated as : 		Real Profit = revenue - cost  
    ...
    select Branch,(revenue-cost) as Real_Profit from Bank_branch_PL
...
# 8) IF Real profit is calculated as : 		Real  Profit = revenue - cost  
	...
	select Branch,(revenue-cost) as Real_Profit,sum(estimated_profit) as estimated_profit
    from Bank_branch_PL
    group by Branch
...
select Product,Banker,Branch,(revenue - cost)
from Bank_branch_PL
...
select Branch,Product,(revenue - cost)
from bank_branch_pl
where (revenue - cost) > 0
group by Branch
Order by (revenue - cost) LIMIT 1;
...
select branch,product,(revenue-cost) as total_calculated_profit
from bank_branch_pl
where (revenue-cost) > Estimated_profit and revenue > cost;
...
select product,(revenue-cost),Estimated_profit
from bank_branch_pl
where (revenue-cost)-Estimated_profit > 0.1 and revenue > cost;
...
where revenue>cost*0.7 and (revenue-cost)*0.7 > Estimated_profit
